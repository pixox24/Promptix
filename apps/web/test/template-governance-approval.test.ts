...
test('approval panel covers plan groups, deletion warning, retry and rollback guards', async () => {
  ...
  assert.match(source, /删除操作不可回滚/); assert.match(source, /审批备注（可选）/);
  assert.doesNotMatch(source, /confirmation !== '永久删除'/); assert.doesNotMatch(source, /!note\.trim\(\)/);
