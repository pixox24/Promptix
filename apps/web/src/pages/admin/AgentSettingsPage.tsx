...
export function AgentSettingsPage({ canManage = false }: { canManage?: boolean }) {
  ...
  useEffect(() => { fetchGovernanceAgentConfig().then((value) => { setConfig(value.config); setModels(value.models); setDefaultVersion(value.defaultPromptVersion); }).catch((error) => setLoadError(error instanceof Error ? error.message : '加载失败')); }, []);
  async function save() {
    setBusy(true);
    try { const result = await saveGovernanceAgentConfig(config); setConfig(result.config); toast(`Agent 配置已保存为规则版本 v${result.version}`, 'success'); }
  ...
  return <section className="mx-auto max-w-3xl space-y-6">
    ...
    <div className="space-y-5 rounded-lg border bg-white p-5 shadow-sm sm:p-6">
      {!canManage && <InlineAlert type="warning">当前账号为只读治理角色，只有 owner 可以修改 Agent 配置。</InlineAlert>}
      {loadError && <InlineAlert type="error">{loadError}</InlineAlert>}
