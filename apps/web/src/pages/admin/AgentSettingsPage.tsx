...
export function AgentSettingsPage() {
  ...
  return <section className="mx-auto max-w-3xl space-y-6">
    ...
    <div className="space-y-5 rounded-lg border bg-white p-5 shadow-sm sm:p-6">
      <label className="block text-sm font-medium">结构化文本模型<select className="mt-2 w-full rounded-lg border px-3 py-2 text-sm" value={config.modelId ?? ''} onChange={(event) => setConfig({ ...config, modelId: event.target.value || null })}><option value="">使用默认模型</option>{models.map((model) => <option key={model.id} value={model.id}>{model.name} · {model.modelId}</option>)}</select><span className="mt-1 block text-xs text-slate-500">只展示启用且支持 text + structured_output 的模型。</span></label>
      <label className="block text-sm font-medium">Prompt 版本<input className="mt-2 w-full rounded-lg border px-3 py-2 text-sm" value={config.promptVersion} onChange={(event) => setConfig({ ...config, promptVersion: event.target.value })}/><span className="mt-1 block text-xs text-slate-500">内置版本：{defaultVersion}。修改后会显示在运行记录中。</span></label>
      <label className="block text-sm font-medium">系统提示词<textarea className="mt-2 min-h-64 w-full rounded-lg border px-3 py-2 text-sm leading-6" value={config.systemPrompt} onChange={(event) => setConfig({ ...config, systemPrompt: event.target.value })} placeholder="留空使用内置治理系统提示词"/><span className="mt-1 block text-xs text-slate-500">留空使用内置提示词；自定义内容会随规则版本保存并冻结到后续运行。</span></label>
      <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-slate-500">保存会生成新的治理规则版本，不影响已排队运行。</p><button type="button" disabled={busy} onClick={save} className="w-full whitespace-nowrap rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 sm:w-auto">{busy ? '保存中…' : '保存 Agent 配置'}</button></div>
