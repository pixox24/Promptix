...
export const confirmTemplateTaxonomy = (id: string, expectedVersion: number) => api<{ currentVersion: number; taxonomyReviewStatus: string }>(`/api/admin/templates/${encodeURIComponent(id)}/taxonomy-confirm`, { method: 'POST', body: JSON.stringify({ expectedVersion, idempotencyKey: crypto.randomUUID() }) });
export const requestTemplatePublish = (id: string, expectedVersion: number) => api<{ changeSetId: string; status: 'awaiting_approval' }>(`/api/admin/templates/${encodeURIComponent(id)}/publish`, { method: 'POST', body: JSON.stringify({ expectedVersion, idempotencyKey: crypto.randomUUID() }) });
