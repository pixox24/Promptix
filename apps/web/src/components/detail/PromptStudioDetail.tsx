...
export function PromptStudioDetail({ template, similarItems, similarRequestId, similarUnavailable, generationRecommendationRequestId, onRecommendationInvalid, initialDraft, favorite, drafts, onFavorite, onSaveDraft, onDeleteDraft, onToast }: { template:PromptTemplate; similarItems:SimilarTemplateViewItem[]; similarRequestId:string|null; similarUnavailable:boolean; generationRecommendationRequestId?:string; onRecommendationInvalid:()=>void; initialDraft?:SavedDraft; favorite:boolean; drafts:SavedDraft[]; onFavorite:()=>void; onSaveDraft:(draft:DraftInput)=>string; onDeleteDraft:(id:string)=>void; onToast:(message:string,type?:'success'|'error'|'info')=>void }) {
  ...
  return <>
    ...
    {similarUnavailable&&<section className="detail-similar-bottom mt-8 rounded-xl border border-slate-200 bg-slate-50 px-4 py-5 text-center"><h2 className="text-sm font-semibold text-slate-700">相似模板暂不可用</h2><p className="mt-1 text-xs text-slate-500">推荐服务暂时没有响应，请稍后再试。</p></section>}
