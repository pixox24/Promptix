...
export function PromptStudioDetail({ template, similarTemplates, initialDraft, favorite, drafts, onFavorite, onSaveDraft, onDeleteDraft, onToast }: { template:PromptTemplate; similarTemplates:PromptTemplate[]; initialDraft?:SavedDraft; favorite:boolean; drafts:SavedDraft[]; onFavorite:()=>void; onSaveDraft:(draft:DraftInput)=>string; onDeleteDraft:(id:string)=>void; onToast:(message:string,type?:'success'|'error'|'info')=>void }) {
  ...
  const requestNavigation=(nextTemplate:PromptTemplate,event:MouseEvent<HTMLAnchorElement>)=>{if(!shouldProtectTemplateNavigation(event,isDirty))return;event.preventDefault();pendingTriggerRef.current=event.currentTarget;setPendingTemplate(nextTemplate)};
  const cancelNavigation=useCallback(()=>{setPendingTemplate(null);window.setTimeout(()=>pendingTriggerRef.current?.focus(),0)},[]);
  const openPending=(saveFirst:boolean)=>{if(!pendingTemplate)return;if(saveFirst)persistDraft();const target=pendingTemplate;setPendingTemplate(null);navigate(`/template/${target.id}`)};

  return <>
    <div className="template-detail-layout">
      <SimilarTemplateRail templates={similarTemplates.slice(0,2)} label="左侧相似模板" className="similar-template-rail-left-tall" onNavigateRequest={requestNavigation}/>
      <SimilarTemplateRail templates={similarTemplates.slice(0,2)} label="左侧相似模板" paged pageSize={1} className="similar-template-rail-left-short" onNavigateRequest={requestNavigation}/>
      ...
      <SimilarTemplateRail templates={similarTemplates} label="右侧相似模板" paged className="similar-template-rail-single-tall" onNavigateRequest={requestNavigation}/>
      <SimilarTemplateRail templates={similarTemplates} label="右侧相似模板" paged pageSize={1} className="similar-template-rail-single-short" onNavigateRequest={requestNavigation}/>
      <SimilarTemplateRail templates={similarTemplates.slice(2,4)} label="右侧相似模板" className="similar-template-rail-right-tall" onNavigateRequest={requestNavigation}/>
      <SimilarTemplateRail templates={similarTemplates.slice(2,4)} label="右侧相似模板" paged pageSize={1} className="similar-template-rail-right-short" onNavigateRequest={requestNavigation}/>
    </div>
    {similarTemplates.length>0&&<section className="detail-similar-bottom mt-12"><h2 className="mb-4 text-base font-semibold text-slate-800">相似模板</h2><div className="detail-similar-mobile-row -mx-4 flex gap-3 overflow-x-auto px-4 pb-3">{similarTemplates.map(item=><div key={item.id} className="w-40 shrink-0"><SimilarTemplateCompactCard template={item} onNavigateRequest={requestNavigation}/></div>)}</div><div className="detail-similar-grid"><TemplateGrid templates={similarTemplates} onNavigateRequest={requestNavigation}/></div></section>}
