...
export function GovernanceInspector({ detail, canManage }: { detail: GovernanceTemplateDetail | null; canManage: boolean }) {
  ...
  return <aside className="overflow-auto border-l border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-2"><div><p className="text-xs text-slate-500">当前模板</p><h2 className="mt-1 font-semibold text-slate-900">{detail.template.name}</h2></div><span className="rounded bg-slate-100 px-2 py-1 text-xs">v{detail.template.currentVersion}</span></div>
    ...
    <GovernanceApprovalPanel approval={detail.approval as ApprovalModel} action={proposal?.action} canManage={canManage}/><section className="mt-5 border-t pt-4"><h3 className="text-sm font-semibold">版本历史</h3><p className="mt-2 text-xs text-slate-500">已记录 {detail.history.length} 个不可变版本</p></section></aside>;
}
