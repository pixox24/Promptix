...
export function GovernanceRulePanel({ onClose, canManage }: { onClose: () => void; canManage: boolean }) {
  ...
  useEffect(() => { fetchActiveGovernanceRules().then((value) => setRules(value.rules)).catch((error) => setLoadError(error.message)); }, []);
  if (!rules) return <aside className="absolute right-0 top-0 z-30 h-full w-96 border-l bg-white p-5 shadow-xl"><button onClick={onClose}>关闭</button>{loadError ? <InlineAlert type="error" className="mt-4">{loadError}</InlineAlert> : <p>正在加载规则…</p>}</aside>;
  ...
  return <aside className="absolute right-0 top-0 z-30 h-full w-full max-w-96 overflow-auto border-l bg-white p-5 shadow-xl"><div className="flex justify-between"><div><h2 className="font-semibold">治理规则</h2><p className="text-xs text-slate-500">保存为新版本，历史运行不受影响</p></div><button onClick={onClose}>关闭</button></div><div className="mt-5 space-y-3">
    {!canManage && <InlineAlert type="warning" className="text-xs">只读权限：修改规则需要 owner 账号。</InlineAlert>}
    ...
    {canManage && <button onClick={async () => { try { await saveActiveGovernanceRules(rules); toast('规则新版本已保存', 'success'); } catch (error) { toast(error instanceof Error ? error.message : '保存失败', 'error'); } }} className="w-full rounded bg-violet-600 px-3 py-2 text-sm text-white">保存为新版本</button>}
  </div></aside>;
}
