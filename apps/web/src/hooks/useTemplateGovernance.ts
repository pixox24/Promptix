...
export function useTemplateGovernance() {
  ...
  useEffect(() => { const requestState = parseGovernanceUrl(new URLSearchParams(requestKey)); const controller = new AbortController(); setStatus('loading'); Promise.all([fetchGovernanceTemplates(serializeGovernanceUrl(requestState), controller.signal), fetchGovernanceQueues(controller.signal)]).then(([next, counts]) => { setPage(next); setQueues(counts); setStatus(next.items.length ? 'ready' : Object.keys(requestState.query).length > 1 ? 'filtered-empty' : 'empty'); }).catch((error) => { if (error.name !== 'AbortError') setStatus(error.status === 403 ? 'forbidden' : navigator.onLine ? 'failed' : 'offline'); }); return () => controller.abort(); }, [requestKey]);
