...
export async function persistGovernancePlan(runId: string, proposals: PlannedProposal[]) {
  return db.transaction(async (tx) => {
    ...
    if (!proposals.length) {
      await tx.update(agentRuns).set({ status: 'succeeded', finishedAt: new Date() }).where(eq(agentRuns.id, run.id));
      return { runId, changeSetId: null, proposals: 0 };
    }
    ...
    await tx.update(agentRuns).set({ status: approval ? 'awaiting_approval' : 'planned', finishedAt: new Date() }).where(eq(agentRuns.id, run.id));
    return { runId, changeSetId: changeSet.id, proposals: created.length };
  });
}
