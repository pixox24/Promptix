...
export async function persistGeneratedOutput(
  ...
  const raw = (output as { images?: RawImage[] } | null)?.images ?? [];
  ...
  for (const [index, image] of raw.entries()) {
    ...
    if (image.b64_json) {
      ...
      if (!response.ok) {
        throw new Error(
          `Unable to persist generated image (${response.status})`,
        );
      }
  ...
  if (!images.length) {
    throw new Error('Image provider returned no persistable images');
  }

  if (templateId) {
    const recorded = await db.update(generationJobs)
      .set({ usageRecordedAt: new Date() })
      .where(and(
        eq(generationJobs.id, jobId),
        isNull(generationJobs.usageRecordedAt),
      ))
    ...
    await recordRecommendationGenerationSuccessSafely({
      jobId,
      templateId,
